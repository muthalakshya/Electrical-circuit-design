...
function checkControl() {
  ...
  for (i = 0; i < len.length; i++) {
    ...
    if ((ab == "dcu" && ba == "redu") || (ba == "dcu" && ab == "redu")) {
      ...
      cnt += 1;
    } else if ((ab == "rd" && ba == "common") || (ba == "rd" && ab == "common")) {
      cnt += 1;
...
function jsStart() {
  jsPlumb.ready(function () {
    ...
    $("#cc").click(() => {
      var ls = ["redu",  "ru","redd"];
      var x_ls = ["x0","x1","x2"]
      endpointFunction(["dcu","but1u","but2u","but3u"], red,"8",1)
      endpointFunction(["dcd"], black,"8",4)
      endpointFunction(["common","but1d","but2d","but3d","rd"], black,"8",1)
      endpointFunction(["negPlc"], black,"8",1)
      endpointFunction(ls, red,"8",1);
      endpointFunction(["x0"], green,"8",1);
      endpointFunction(["x1"], orange,"8",1);
      endpointFunction(["x2"], purple,"8",1);
...
//           if(result.value=="a" || result.value=="b" || result.value=="c"){
//               let userInput = result.value;
//               results[buttonId] = userInput;
